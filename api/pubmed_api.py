...
def search_pubmed(query, max_results=10, max_retries=3, retry_delay=1):
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                print(f"PubMed API超时，正在重试 ({attempt + 1}/{max_retries})...")
            ...
            if attempt < max_retries - 1:
                print(f"PubMed API请求错误: {e}，正在重试 ({attempt + 1}/{max_retries})...")
                time.sleep(retry_delay)
            else:
                print(f"PubMed API多次请求错误，放弃重试: {e}")
                return []
        except Exception as e:
            if attempt < max_retries - 1:
                print(f"PubMed API错误: {e}，正在重试 ({attempt + 1}/{max_retries})...")
                time.sleep(retry_delay)
            else:
